...
describe('PgnNotation with Annotations', () => {
  ...
  describe('Loading PGN with annotations', () => {
    ...
    it('should parse a PGN with circle annotations', () => {
      ...
      expect(firstMove.whiteAnnotations?.circles?.some(c => c.square === 'd4' && c.color === '#ff0000')).toBe(true);
      expect(firstMove.whiteAnnotations?.circles?.some(c => c.square === 'e4' && c.color === '#00ff00')).toBe(true);
      
      // Check first move black (1... Nf6)
      expect(firstMove.blackAnnotations?.circles).toHaveLength(2);
      expect(firstMove.blackAnnotations?.circles?.some(c => c.square === 'f6' && c.color === '#00ff00')).toBe(true);
      expect(firstMove.blackAnnotations?.circles?.some(c => c.square === 'e6' && c.color === '#ffff00')).toBe(true);
    });
  ...
  describe('Exporting PGN with annotations', () => {
    ...
    it('should preserve standard PGN format when no annotations are present', () => {
      ...
      expect(annotatedPgn.replace(/\s+/g, ' ').trim()).toBe(standardPgn.replace(/\s+/g, ' ').trim());
    });
  });
    ...
    it('should maintain annotation integrity in round-trip', () => {
      ...
      for (let i = 0; i < originalMoves.length; i++) {
        ...
        if (original.whiteAnnotations) {
          ...
          expect(second.whiteAnnotations?.textComment?.trim()).toBe(original.whiteAnnotations.textComment?.trim());
        }
        ...
        if (original.blackAnnotations) {
          ...
          expect(second.blackAnnotations?.textComment?.trim()).toBe(original.blackAnnotations.textComment?.trim());
        }
      }
    });
  });
